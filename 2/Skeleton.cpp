...
public:
	void create() {
		...
		vec3 n = { -D.y, D.x, 0 };
		n = normalize(n);
		mat4 t = RotationMatrix(dot(n, {0, 1, 0}), {0, 0, 1})*TranslateMatrix(pos)*TranslateMatrix(n);
		obj.transform(t);
		obj.updateGPU();
	}
	...
	void animate(float dt) {	
		...
		vec3 v0 = normalize(spline.r(tau, true));
		vec3 v1 = normalize(spline.r(tau+dtau, true));
